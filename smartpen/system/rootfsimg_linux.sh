...
if [ "${FSTYPE}" = "jffs2" ]; then
	...
	#COUNT_SIZE=`expr $EMMC_ROOTFS_SIZE \* 1024 \* 1024`
	${ROOTFS_DIR}/../buildroot-intermediate/host/sbin/mkfs.ext2 -d ${ROOTFS_DIR} -r 1 -N 0 -m 5 -L "" -O ^64bit ${ROOTFS_IMG} "${EMMC_ROOTFS_SIZE}M"
    fi
else
    echo "Unsupported fs type!" >&2
fi
